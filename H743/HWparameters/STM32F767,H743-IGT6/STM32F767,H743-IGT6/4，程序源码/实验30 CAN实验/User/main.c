...
int main(void)
{
    ...
    lcd_init();                                 /* 初始化LCD */
    key_init();                                 /* 初始化按键 */
    can_init(CAN_SJW_1TQ, CAN_BS2_6TQ, CAN_BS1_11TQ, 6, CAN_MODE_LOOPBACK);             /* CAN初始化, 环回模式, 波特率500Kbps */
    ...
    lcd_show_string(30, 230, 200, 16, 16, "Receive Data:", BLUE);
    ...
    while (1)
    {
        ...
        else if (key == WKUP_PRES) /* WK_UP按下, 改变CAN的工作模式 */
        {
            ...
            if (mode == 0)
            {
                can_init(CAN_SJW_1TQ, CAN_BS2_6TQ, CAN_BS1_11TQ, 6, CAN_MODE_NORMAL);       /* CAN初始化, 普通(0)/回环(1)模式, 波特率500Kbps */
            }
            else if (mode == 1)
            {
                can_init(CAN_SJW_1TQ, CAN_BS2_6TQ, CAN_BS1_11TQ, 6, CAN_MODE_LOOPBACK);     /* CAN初始化, 普通(0)/回环(1)模式, 波特率500Kbps */
            }
