...
│
├── YOLOv8forSTM/                # 模型训练
│   ├── train_kd.py              # 知识蒸馏训练脚本（两阶段）
│   ├── create_v4.py             # 数据集重构（删类 + 降采样）
...
1. 用公开数据集训练基线模型（YOLO11l 教师 + YOLO11n 学生知识蒸馏）
2. 自制冰箱场景纯净数据集 → 重训练
...
| 项目 | 状态 |
|------|------|
| 数据集 | `fridge_33_v4` — 31 类（删除 clam/squid），水果类降采样平衡 |
| 教师模型 | YOLO11l @ RTX 3080 云服务器，batch=14，100 epoch 训练中 |
| 训练框架 | `YOLOv8forSTM/train_kd.py`（两阶段知识蒸馏） |
| 数据处理 | `YOLOv8forSTM/create_v4.py`（v3→v4：删类 + 密度降采样） |
| 下一步 | Stage 2 蒸馏 → YOLO11n 学生模型
