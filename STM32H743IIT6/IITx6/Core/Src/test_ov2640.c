 ...
 *
 * 使用方式：
 *   test_config.h 中 #define TEST_PHASE_OV2640_SNAPSHOT
 *   小板复位后自动拍一张照片，发到 PC
 */
...
#define OV5640_I2C_ADDR  0x78   /* 7-bit=0x3C */
...
/* OV5640 16-bit 寄存器读 (HAL_I2C_Mem_Read 支持 16-bit 地址) */
static uint8_t ov5640_read(uint16_t reg)
{
    uint8_t val = 0;
    HAL_I2C_Mem_Read(&hi2c1, OV5640_I2C_ADDR, reg,
...
{
    printf("\r\nPhase 2b: OV5640 Snapshot → PC\r\n");

    /* 1. I2C 探测 OV5640 PID (reg 0x300A/0x300B → 0x56 0x40) */
    uint8_t pid = ov5640_read(0x300A);
    uint8_t ver = ov5640_read(0x300B);
    printf("[I2C] OV5640 PID=0x%02X VER=0x%02X\r\n", pid, ver);
    if (pid != 0x56)
    {
        printf("[FAIL] Camera not found! (expected PID=0x56)\r\n");
